...
namespace TableSO.Scripts.Generator
{
    public class CsvTableGenerator
    {
        ...
        private static bool ValidateIDField(string idFieldName, string idFieldType, string fileName)
        {
            ...
            {
                Debug.LogError($"[CsvTableSO] {fileName}.csv: First field must be 'ID'. Current: {idFieldName}");
                return false;
            }
        ...
        private static bool ValidateFieldTypes(string[] fieldTypes, string fileName)
        {
            for (int i = 0; i < fieldTypes.Length; i++)
            {
                ...
                if (IsArrayType(fieldType))
                {
                    ...
                    {
                        Debug.LogError($"[CsvTableSO] {fileName}.csv: Unsupported array element type: {elementType} (field {i + 1})");
                        return false;
                    }
                }
                // Single type case
                ...
                {
                    Debug.LogError($"[CsvTableSO] {fileName}.csv: Unsupported type: {fieldType} (field {i + 1})");
                    return false;
                }
            }
        ...
        private static bool ValidateEnumType(string enumTypeName)
        {
            ...
            if (userDefinedEnums.Count > 0)
            {
                if (userDefinedEnums.Count == 1)
                {
                    ...
                    Debug.Log($"[CsvTableSO] Enum type '{enumTypeName}' confirmed (full name: {foundEnum.FullName})");
                    return true;
                }
                else
                {
                    ...
                    if (continueAnyway)
                    {
                        Debug.Log($"[CsvTableSO] Using enum type: {userDefinedEnums[0].FullName}");
                        return true;
                    }
                    else
                    {
                        Debug.Log("[CsvTableSO] User cancelled generation due to multiple enum types");
                        return false;
                    }
                }
            }
            ...
            Debug.LogWarning($"[CsvTableSO] Cannot find user-defined enum type '{enumTypeName}' in current assemblies");
            ...
            {
                Debug.Log($"[CsvTableSO] User choice: Continue with enum type '{enumTypeName}' assumption");
                return true;
            }
            else
            {
                Debug.Log("[CsvTableSO] User choice: Cancel generation");
                return false;
            }
        }
        ...
        private static List<Type> FindUserDefinedEnumTypes(string enumTypeName)
        {
            ...
            foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                catch (Exception e)
                {
                    ...
                    Debug.LogWarning($"[CsvTableSO] Error loading types from assembly {assembly.FullName}: {e.Message}");
                }
            }
        ...
        private static void GenerateDataClass(string className, string[] fieldNames, string[] fieldTypes)
        {
            ...
            classCode.AppendLine("/// Made by TableSO CsvTableGenerator");
            classCode.AppendLine("/// </summary>");
            classCode.AppendLine();
